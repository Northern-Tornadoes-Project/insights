...
export function Actions({ id, hidden }: ActionsProps) {
	...
	const handleContinueClick = async () => {
		try {
			const response = await fetch(`/action/lidar/set-hidden?id=${id}&hidden=${!hidden}`, {
				method: 'POST'
			});
	...
	const handleCopyLink = async () => {
		await navigator.clipboard.writeText(`${window.location.origin}/lidar/${id}`);
		toast.info('Link copied to clipboard');
	};

	const handleCopyIframe = async () => {
		await navigator.clipboard.writeText(
			`<iframe src="${window.location.origin}/lidar/frame/${id}" width="100%" height="100%"></iframe>`
		);
			...
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						...
						the LiDAR scan from being publicly accessible.
					</AlertDialogDescription>
				</AlertDialogHeader>
				<AlertDialogFooter>
